function SpirographTool() {
  ...
  this.calcArray = function (xPos, yPos) {
    ...
    for (var i = 0; i < rotationNum; i++) {
      ...
      array.push([
        width / 2 +
          (xPos - width / 2) * cos((i * PI) / (rotationNum / 2)) -
          (yPos - height / 2) * sin((i * PI) / (rotationNum / 2)),
        height / 2 +
          (xPos - width / 2) * sin((i * PI) / (rotationNum / 2)) +
          (yPos - height / 2) * cos((i * PI) / (rotationNum / 2)),
      ]);
    }
    return array;
  };
    ...
    if (mouseIsPressed) {
      ...
      if (drawing == false) {
        ...
        for (var i = 0; i < positionArray.length; i++) {
          ...
          line(
            previousPositionArray[i][0],
            previousPositionArray[i][1],
            positionArray[i][0],
            positionArray[i][1]
          );
        }
